...
def main(round, game_mode, default_points, index):
    ...
    try:
        with open(file="game_mode.txt", mode="r") as file:
            text = file.read()

        game_mode = int(text)
        player_points = game_mode
    ...
    player1_label = tkinter.Label(bg=BACKGROUND_COLOR, text=player1_name, font=FONT)
    player2_label = tkinter.Label(bg=BACKGROUND_COLOR, text=player2_name, font=FONT)

    # TODO fetch the game mode from welcome and bind here

    player1_points = tkinter.Label(bg=BACKGROUND_COLOR, text=text, font=POINTS_FONT)
    player2_points = tkinter.Label(bg=BACKGROUND_COLOR, text=text, font=POINTS_FONT)
    entry_text = tkinter.Label(bg=BACKGROUND_COLOR, text="enter points here", font=("Helvetica", "12",))

    player1_entry = tkinter.Entry()
    player2_entry = tkinter.Entry()
    enter_button_1 = tkinter.Button(text="Enter", font=FONT)
    enter_button_2 = tkinter.Button(text="enter")
    new_game = tkinter.Button(text="Start new game", bg=BG_COLOR, font=FONT)
    ...
    game_mode_label.grid(row=0, column=0,columnspan=2,sticky="e")
    ...
    def show_winner(winner):
        ...
        empty_command()
        enter_button_1.config(command=empty_command)
    ...
    points1 = player_points
    points2 = player_points
